...
//合并工具库包
fis.media('prod').match('/modules/lib/**' + matchModFiles, {
    packTo: '/modules/pkg/lib.js'
});

//合并配置包
fis.media('prod').match('/modules/conf/**' + matchModFiles, {
    packTo: '/modules/pkg/conf.js'
});

//合并组件包
fis.media('prod').match('/modules/coms/**' + matchModFiles, {
    packTo: '/modules/pkg/coms.js'
});

//合并单个模块包（按需加载）
fis.media('prod').match('/modules/buss/(*)/**' + matchModFiles, {
    packTo: '/modules/pkg/mod-$1.js'
});

//合并单个页面包（按需加载）
fis.media('prod').match('/modules/page/(*)/**' + matchModFiles, {
    packTo: '/modules/pkg/page-$1.js'
});

//合并单个应用包（按需加载）
fis.media('prod').match('/modules/app/(*).*' + matchModFiles, {
    packTo: '/modules/pkg/app-$1.js'
});
